...
public class SearchService implements ISearchService {
    ...
    public List<Memory> findByTag(String tag) throws SQLException {
        ...
        ResultSet rs = stmt.executeQuery("select  m.memoryid, m.title, m.description, m.memorylocation , m.datem, m.privacy, m.mainpicture from tag t join tag_memory tm on tm.tag_id=t.tag_id join memory m on m.memoryid=tm.memoryid where tag_name = '" + tag + "'");
        Memory memory;
    ...
    public List<FriendModel> findFriends(String word, int id) throws SQLException {
        ...
        Statement stmt = con.createStatement();
        ResultSet rs = stmt.executeQuery("select username, datefriends, name, profile_picture, city, country from (select username, datefriends, name, profile_picture, city, country from (select r.user2, datefriends from user_table u join relationship r on u.user_id = r.user1 where user_id = " + id + " ) join user_table t on user_id=user2 UNION select username, datefriends, name, profile_picture, city, country from (select r.user1, datefriends from user_table u join relationship r on u.user_id = r.user2 where user_id =" + id + ") join user_table t on user_id=user1) where name like " + "'%" + word + "%'");
    ...
    public List<FriendModel> findNewFriends(String word, int id) throws SQLException {
        ...
        Statement stmt = con.createStatement();
        ResultSet rs = stmt.executeQuery("select username,date_of_birth, name, profile_picture, city, country from ((select username,date_of_birth, name, profile_picture, city, country from user_table) MINUS (select username,date_of_birth, name, profile_picture, city, country from (select r.user2, datefriends from user_table u join relationship r on u.user_id = r.user1 where user_id = " + id + " ) join user_table t on user_id=user2 UNION select username,date_of_birth, name, profile_picture, city, country from (select r.user1, datefriends from user_table u join relationship r on u.user_id = r.user2 where user_id = " + id + ") join user_table t on user_id=user1)) where name like '%" + word + "%' ");
        FriendModel friend ;
        List<FriendModel> friends = new LinkedList<>();
